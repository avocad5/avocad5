...
for(var i = 0; i<projectname.length; i++){ //버튼 생성
  if(i<2){
  document.getElementsByClassName('project')[0].innerHTML += `<a href="${projectlink[i]}"><div><p class="pname"><strong>${projectname[i]}</strong></p><p class="pment">${projectment[i]}</p></div></a>`
} else{
   document.getElementsByClassName('project')[0].innerHTML += `<a href="${projectlink[i]}"><div><p class="pname"><strong>${projectname[i]}</strong></p><p class="pment">${projectment[i]}</p></div></a>`
}
}
